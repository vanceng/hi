...
def has_sin?(string)
  string.match(/(\b\d{3}-\d{3}-\d{3}\b)/) ? true : false
end
...
def grab_sin(string)
  string.match(/(\b\d{3}-\d{3}-\d{3}\b)/).to_s
end
...
def hide_all_sins(string)
  p string.gsub(/\b\d{3}-\d{3}\b/, "XXX-XXX").to_s 
  # puts string = string.gsub(/(\b^\d{3}-\d{3}\b)/, "XXX-XXX") 
end
